...
def _json_error(message, status_code=400, *, code=None, detail=None):
...
def api_upload():
    if "files" not in request.files:
        return _json_error("No files uploaded", 400, code="missing_files")
    ...
    try:
        ...
        db.session.rollback()
        return _json_error(
            "Duplicate records detected. No new data were saved because the upload contains entries that already exist.",
            400,
        ...
        return _json_error(
            "Unexpected server error while processing upload. Please try again later.",
            500,
