...
def api_upload():
    ...
    for f in files:
        ...
        existing_import = Import.query.filter_by(filename=fname).first()
        if existing_import:
            existing_import.imported_on = str(date.today())
        else:
            db.session.add(Import(filename=fname, imported_on=str(date.today())))
